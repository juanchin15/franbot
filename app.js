...
const { generateImage, generateInvite, cleanNumber, checkEnvFile, createClient, isValidNumber } = require('./controllers/handle')
...
const listenMessage = () => client.on('message', async msg => {
    ...
    */
    ...
    if (step) {
        ...
        if (response.delay && response.media) {
            if(response.media === 'qr-invite'){
                const fileName = `${response.media}_${from.split('@')[0]}.png`
                generateInvite(randomString(), fileName)
                setTimeout(function() {
                    sendMedia(client, from, fileName)    
                }, response.delay);  
            }
            else{
                setTimeout(() => {
