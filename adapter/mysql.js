...
const DATABASE_NAME = process.env.SQL_DATABASE || 'frandb'
...
getData = (message = '', callback) => connection.query(
    ...
    const [response] = results
    getKey = (response) => (response != null) ? response.option_key : 'DEFAULT'
...
saveMessageMysql = ( message, trigger, number ) => new Promise((resolve,reejct) => {
    try {
        connection.query(
        `INSERT INTO ${DATABASE_NAME}.message  `+"( `message`, `date`, `trigger`, `number`)"+` VALUES ('${message}','${date}','${trigger}', '${number}')` , (error, results) => {
            if(error) {
